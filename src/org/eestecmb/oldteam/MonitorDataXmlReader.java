...
public class MonitorDataXmlReader extends DefaultHandler
{
	...
	private boolean isMonitorInfo = false;
	...
	public void characters(char ch[], int start, int length)
	{
		...
		if (isMonitorInfo && isQuotas)
		{
			...
			if (isQuota)
			{
				...
				if (isShortDescription)
					tmpShortDescription = value;
				else if (isDescription)
					tmpDescription = value;
				else if (isUnit)
					tmpUnit = value;
				else if (isAvailable)
					tmpAvailable = Double.parseDouble(value);
				else if (isUsed)
				{
		...
		else if (isMonitorInfo && isPPStatus)
		{
			if (isAirTimeDate)
			{
		...
		else if (isMonitorInfo && !isQuotas && !isPPStatus)
		{
			// stanje
			/*
			 * <Stanje>0</Stanje> <MonetaLocenRacun>0</MonetaLocenRacun>
			 * <Moneta>0.00</Moneta> <PorabaLocenRacun>0</PorabaLocenRacun>
			 * <Poraba>0.42</Poraba>
			 * <HasAdditionalAccount>false</HasAdditionalAccount>
			 */
			if (isUserType)
			{
				if (value.equalsIgnoreCase("postpaid"))
					monitorResult.prePaid = false;
				else
					monitorResult.prePaid = true;
			}
			else if (isStanje)
				monitorResult.stanje = Double.parseDouble(value);
			else if (isStanje)
				monitorResult.stanje = Double.parseDouble(value);
			else if (isStanje)
				monitorResult.stanje = Double.parseDouble(value);
				...
				monitorResult.stanje = Double.parseDouble(value);
			else if (isMonetaLocenRacun)
				monitorResult.monetaLocenRacun = Double.parseDouble(value);
			else if (isMoneta)
				monitorResult.moneta = Double.parseDouble(value);
			else if (isPorabaLocenRacun)
				monitorResult.porabaLocenRacun = Double.parseDouble(value);
			else if (isPoraba)
				monitorResult.poraba = Double.parseDouble(value);
			else if (isHasAdditionalAccount)
				monitorResult.hasAdditionalAccount = Boolean.parseBoolean(value);
		}
	}
